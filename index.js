...
app.get("/verify2fa", authenticatorTwoFA, (req, res) => {
  res.send({ error: false, message: "2fa code is correct" });
});
...
app.post("/online", authenticator, (req, res) => {
  let data = req.body;
  users[req.user.username]["public"]["active"] = data.active;
});
  ...
  if (username in users) {
    ...
    bcrypt.compare(password, hash, (err, result) => {
      ...
      if (is2faEnabled) {
        ...
        return res.json({
          error: true,
          message: "Two factor auth code required",
        });
      }
      const token = jwt.sign({ username }, secretKey, { expiresIn: "24h" });
      return res.json({ error: false, token });
